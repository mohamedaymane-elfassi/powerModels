...
cvLassoFit = [cvLassoModel[i].fit(cvLinearRegressionVariables, cvTransPrice) for i in range(0, len(alphas))]
cvForecastPrices = [cvLassoFit[i].predict(cvTestLinearRegressionVariables) for i in range(0, len(alphas))]
#####################################
...
###############################
alphaMin = alphas[13]
lassoModel = [linear_model.Lasso(alpha = alphaMin) for j in range(0, 1456*24)]
lassoFit = [lassoModel[j].fit(trainLinearRegressionVariables[j], trainTransPrice[j]) for j in range(0, 1456*24)]
forecastPrices = [lassoFit[j].predict(testLinearRegressionVariables[j:j+1]) for j in range(0, 1456*24)]
#####################################
#####################################

#####################################
#####################################
coeffOccurence = [[(lassoFit[k+j*24].coef_ != 0).astype(int) for j in range(0, 1456)] for k in range(0,24)]
...
sns_plot.figure.savefig('C:\\Users\\Aymane\\Documents\\Power markets\\Books\\Weron - Modeling and Forecasting Electricity Loads and Prices\\Chap4\\Implementation\\models\\C4-lasso\\c_occ.png')
#####################################
#####################################
coeffOccurences90pct = np.zeros((96,24)).astype(int)
coeffOccurences90pct[coeffOccurences >= 0.1*1456] =  coeffOccurences[coeffOccurences >= 0.1*1456]
plt.subplots(figsize=(20,80))
sns_plot = sns.heatmap(coeffOccurences90pct, annot = True, fmt="", cmap='RdYlGn', linewidths=1.5)
sns_plot.figure.savefig('C:\\Users\\Aymane\\Documents\\Power markets\\Books\\Weron - Modeling and Forecasting Electricity Loads and Prices\\Chap4\\Implementation\\models\\C4-lasso\\c_occ90pct.png')
#####################################
#####################################
coeffOccurences80pct = np.zeros((96,24)).astype(int)
coeffOccurences80pct[coeffOccurences >= 0.2*1456] =  coeffOccurences[coeffOccurences >= 0.2*1456]
plt.subplots(figsize=(20,80))
sns_plot = sns.heatmap(coeffOccurences80pct, annot = True, fmt="", cmap='RdYlGn', linewidths=1.5)
sns_plot.figure.savefig('C:\\Users\\Aymane\\Documents\\Power markets\\Books\\Weron - Modeling and Forecasting Electricity Loads and Prices\\Chap4\\Implementation\\models\\C4-lasso\\c_occ80pct.png')
#####################################
#####################################
coeffOccurences70pct = np.zeros((96,24)).astype(int)
coeffOccurences70pct[coeffOccurences >= 0.3*1456] =  coeffOccurences[coeffOccurences >= 0.3*1456]
plt.subplots(figsize=(20,80))
sns_plot = sns.heatmap(coeffOccurences70pct, annot = True, fmt="", cmap='RdYlGn', linewidths=1.5)
sns_plot.figure.savefig('C:\\Users\\Aymane\\Documents\\Power markets\\Books\\Weron - Modeling and Forecasting Electricity Loads and Prices\\Chap4\\Implementation\\models\\C4-lasso\\c_occ70pct.png')
#####################################
#####################################
coeffOccurences60pct = np.zeros((96,24)).astype(int)
coeffOccurences60pct[coeffOccurences >= 0.4*1456] =  coeffOccurences[coeffOccurences >= 0.4*1456]
plt.subplots(figsize=(20,80))
sns_plot = sns.heatmap(coeffOccurences60pct, annot = True, fmt="", cmap='RdYlGn', linewidths=1.5)
sns_plot.figure.savefig('C:\\Users\\Aymane\\Documents\\Power markets\\Books\\Weron - Modeling and Forecasting Electricity Loads and Prices\\Chap4\\Implementation\\models\\C4-lasso\\c_occ60pct.png')
#####################################
#####################################
coeffOccurences50pct = np.zeros((96,24)).astype(int)
coeffOccurences50pct[coeffOccurences >= 0.5*1456] =  coeffOccurences[coeffOccurences >= 0.5*1456]
plt.subplots(figsize=(20,80))
sns_plot = sns.heatmap(coeffOccurences50pct, annot = True, fmt="", cmap='RdYlGn', linewidths=1.5)
sns_plot.figure.savefig('C:\\Users\\Aymane\\Documents\\Power markets\\Books\\Weron - Modeling and Forecasting Electricity Loads and Prices\\Chap4\\Implementation\\models\\C4-lasso\\c_occ50pct.png')
#####################################
#####################################
