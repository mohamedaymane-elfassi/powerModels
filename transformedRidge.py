...
cvRidgeFit = [cvRidgeModel[i].fit(cvLinearRegressionVariables, cvTransPrice) for i in range(0, len(alphas))]
cvForecastPrices = [cvRidgeFit[i].predict(cvTestLinearRegressionVariables) for i in range(0, len(alphas))]
#####################################
...
###############################
alphaMin = 100
ridgeModel = [Ridge(alpha = alphaMin) for j in range(0, 1456*24)]
ridgeFit = [ridgeModel[j].fit(trainLinearRegressionVariables[j], trainTransPrice[j]) for j in range(0, 1456*24)]
forecastPrice = [ridgeFit[j].predict(testLinearRegressionVariables[j:j+1]) for j in range(0, 1456*24)]
#####################################
#####################################
...
## RETRIEVING ACTUAL PRICE ##
#############################
forecastDataframe['FCST PRICE'] = np.sinh(np.asarray(forecastPrice))*initialCalibrationMAD + initialCalibrationMedian
############################
############################
