...
####################################
linearRegressionInputData = np.array([PdMinus1, PdMinus2, PdMinus7, PdMin, Dsat, Dsun, Dmon, Dhol, PdMinus1_24])
...
####################################
##    APPLYING THE AR1hm MODEL    ##
####################################
####################################
ar1Model = [linear_model.LinearRegression() for j in range(0, 1456*24)]
ar1Fit = [ar1Model[j].fit(trainLinearRegressionVariables[j], trainTransPrice[j]) for j in range(0, 1456*24)]
forecastPrices = [ar1Fit[j].predict(testLinearRegressionVariables[j:j+1]) for j in range(0, 1456*24)]
#####################################
#####################################
