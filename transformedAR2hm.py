...
####################################
PdMax = np.asarray(pd.merge(finalDataset['DAY'], finalDataset.groupby("DAY")["TRANS PRICE"].max().reset_index())['TRANS PRICE'].shift(24))
PdAvg = np.asarray(pd.merge(finalDataset['DAY'], finalDataset.groupby("DAY")["TRANS PRICE"].mean().reset_index())['TRANS PRICE'].shift(24))
hours = np.asarray(finalDataset["HOUR"])
####################################
linearRegressionInputData = np.array([PdMinus1, PdMinus2, PdMinus7, PdMin, Dsat, Dsun, Dmon, Dhol, PdMinus1_24, PdMax, PdAvg, hours])
...
####################################
##     APPLYING THE AR2hm MODEL   ##
####################################
####################################
ar2Model = [linear_model.LinearRegression() for j in range(0, 1456*24)]
ar2Fit = [ar2Model[j].fit(trainLinearRegressionVariables[j], trainTransPrice[j]) for j in range(0, 1456*24)]
forecastPrices = [ar2Fit[j].predict(testLinearRegressionVariables[j:j+1]) for j in range(0, 1456*24)]
#####################################
#####################################
